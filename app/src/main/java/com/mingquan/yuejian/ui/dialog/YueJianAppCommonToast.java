...
public class YueJianAppCommonToast {
  ...
  public YueJianAppCommonToast(Activity activity) {
    init(activity);
  }

  public YueJianAppCommonToast(
      Activity activity, String message, int icon, String action, int actionIcon, long l) {
    _duration = l;
    init(activity);
    setMessage(message);
    setMessageIc(icon);
  ...
  public void setLayoutGravity(int i) {
    if (i != 0) {
      ...
      params.gravity = i;
      int j = (int) YueJianAppTDevice.dpToPixel(16F);
  ...
  public void show() {
    final ViewGroup content =
        (ViewGroup) ((Activity) _toastVw.getContext()).findViewById(android.R.id.content);
    if (content != null) {
      ObjectAnimator.ofFloat(_toastVw, "alpha", 0.0F).setDuration(0L).start();
      content.addView(_toastVw);
      ObjectAnimator.ofFloat(_toastVw, "alpha", 0.0F, 1.0F).setDuration(167L).start();
      _toastVw.postDelayed(new Runnable() {

        @Override
        public void run() {
          ObjectAnimator animator = ObjectAnimator.ofFloat(_toastVw, "alpha", 1.0F, 0.0F);
  ...
  private class ToastView extends FrameLayout {
    ...
    private void init() {
      LayoutInflater.from(getContext()).inflate(R.layout.yue_jian_app_view_base_toast, this, true);

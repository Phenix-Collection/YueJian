 ...
 */
public final class YueJianAppApiClientMgr implements HuaweiApiClient.ConnectionCallbacks, HuaweiApiClient.OnConnectionFailedListener, YueJianAppIActivityResumeCallback, YueJianAppIActivityPauseCallback, YueJianAppIActivityDestroyedCallback {

    /**
     * 单实例
     */
    public static final YueJianAppApiClientMgr INST = new YueJianAppApiClientMgr();
     ...
     * 连接回调
     */
    private List<YueJianAppIClientConnectCallback> connCallbacks = new ArrayList<YueJianAppIClientConnectCallback>();

    /**
     * 注册的静态回调
     */
    private List<YueJianAppIClientConnectCallback> staticCallbacks = new ArrayList<YueJianAppIClientConnectCallback>();
    ...
    private Handler timeoutHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            ...
            if (msg != null && msg.what == APICLIENT_TIMEOUT_HANDLE_MSG && hasConnCallbacks) {
                YueJianAppHMSAgentLog.d("connect time out");
                resetApiClient();
                onConnectEnd(YueJianAppHMSAgent.AgentResultCode.APICLIENT_TIMEOUT);
                return true;
            } else if (msg != null && msg.what == APICLIENT_STARTACTIVITY_TIMEOUT_HANDLE_MSG && hasConnCallbacks) {
                YueJianAppHMSAgentLog.d("start activity time out");
                onConnectEnd(YueJianAppHMSAgent.AgentResultCode.APICLIENT_TIMEOUT);
                return true;
            } else if (msg != null && msg.what == UPDATE_OVER_ACTIVITY_CHECK_TIMEOUT_HANDLE_MSG && hasConnCallbacks) {
                YueJianAppHMSAgentLog.d("Discarded update dispose:hasOverActivity=" +hasOverActivity + " resolveActivity=" + YueJianAppStrUtils.objDesc(resolveActivity));
     ...
     * 私有构造方法
     */
    private YueJianAppApiClientMgr () {
    }
    ...
    public void init(Application app) {
        ...
        YueJianAppActivityMgr.INST.unRegisterActivitResumeEvent(this);
        YueJianAppActivityMgr.INST.registerActivitResumeEvent(this);

        // 注册activity onPause回调
        YueJianAppActivityMgr.INST.unRegisterActivitPauseEvent(this);
        YueJianAppActivityMgr.INST.registerActivitPauseEvent(this);

        // 注册activity onDestroyed 回调
        YueJianAppActivityMgr.INST.unRegisterActivitDestroyedEvent(this);
        YueJianAppActivityMgr.INST.registerActivitDestroyedEvent(this);
    }
     ...
     */
    public void registerClientConnect(YueJianAppIClientConnectCallback staticCallback){
        synchronized (STATIC_CALLBACK_LOCK) {
     ...
     * @param staticCallback 连接回调
     */
    public void removeClientConnectCallback(YueJianAppIClientConnectCallback staticCallback) {
        synchronized (STATIC_CALLBACK_LOCK) {
    ...
    private HuaweiApiClient resetApiClient(){
        synchronized (APICLIENT_LOCK) {
            ...
            YueJianAppHMSAgentLog.d("reset client");
     ...
     */
    public void connect(YueJianAppIClientConnectCallback callback, boolean allowResolve) {

        if (context == null) {
            aSysnCallback(YueJianAppHMSAgent.AgentResultCode.HMSAGENT_NO_INIT, callback);
            return;
        }
        ...
        if (client != null && client.isConnected()) {
            YueJianAppHMSAgentLog.d("client is valid");
            aSysnCallback(YueJianAppHMSAgent.AgentResultCode.HMSAGENT_SUCCESS, callback);
            return;
        } else {
            // client无效，将callback加入队列，并启动连接
            synchronized (CALLBACK_LOCK) {
                YueJianAppHMSAgentLog.d("client is invalid：size=" + connCallbacks.size());
    ...
    private void startConnect() {
        ...
        YueJianAppHMSAgentLog.d("start thread to connect");
        YueJianAppThreadUtil.INST.excute(new Runnable() {
            @Override
            public void run() {
                HuaweiApiClient client =  getApiClient();
                if (client == null) {
                    YueJianAppHMSAgentLog.d("create client");
                    client = resetApiClient();
                }

                YueJianAppHMSAgentLog.d("connect");
                Activity curActivity = YueJianAppActivityMgr.INST.getLastActivity();
    ...
    private void onConnectEnd(final int rstCode) {
        YueJianAppHMSAgentLog.d("connect end:" + rstCode);

        synchronized (CALLBACK_LOCK) {
            // 回调各个回调接口连接结束
            for (YueJianAppIClientConnectCallback callback : connCallbacks) {
        ...
        synchronized (STATIC_CALLBACK_LOCK) {
            // 回调各个回调接口连接结束
            for (YueJianAppIClientConnectCallback callback : staticCallbacks) {
                aSysnCallback(rstCode, callback);
            }
            staticCallbacks.clear();
        }
     ...
     * @param callback 回调
     */
    private void aSysnCallback(final int rstCode, final YueJianAppIClientConnectCallback callback) {
        YueJianAppThreadUtil.INST.excute(new Runnable() {
            @Override
            public void run() {
                HuaweiApiClient client =  getApiClient();
                YueJianAppHMSAgentLog.d("callback connect: rst=" + rstCode + " apiClient=" + client);
    ...
    public void onActivityResume(Activity activity) {
        ...
        if (apiClient != null) {
            YueJianAppHMSAgentLog.d("tell hmssdk: onResume");
            apiClient.onResume(activity);
        }

        // 如果正在解决错误，则处理被覆盖的场景
        YueJianAppHMSAgentLog.d("is resolving:" + isResolving);
        if (isResolving && !PACKAGE_NAME_HIAPP.equals(curAppPackageName)) {
            if (activity instanceof BridgeActivity) {
                resolveActivity = (BridgeActivity)activity;
                hasOverActivity = false;
                YueJianAppHMSAgentLog.d("received bridgeActivity:" + YueJianAppStrUtils.objDesc(resolveActivity));
            } else if (resolveActivity != null && !resolveActivity.isFinishing()){
                hasOverActivity = true;
                YueJianAppHMSAgentLog.d("received other Activity:" + YueJianAppStrUtils.objDesc(resolveActivity));
            }
    ...
    /**
     * connect fail 解决结果回调， 由 YueJianAppHMSAgentActivity 在 onActivityResult 中调用
     * @param result 解决结果
     */
    void onResolveErrorRst(int result) {
        YueJianAppHMSAgentLog.d("result="+result);
        isResolving = false;
        resolveActivity = null;
        hasOverActivity = false;
    ...
    /**
     * YueJianAppHMSAgentActivity 拉起拉了（走了onCreate）
     */
    void onActivityLunched(){
        YueJianAppHMSAgentLog.d("resolve onActivityLunched");
        // 拉起界面回调，移除拉起界面超时
    ...
    public void onConnected() {
        YueJianAppHMSAgentLog.d("connect success");
    ...
    public void onConnectionSuspended(int cause) {
        YueJianAppHMSAgentLog.d("connect suspended");
        connect(new YueJianAppEmptyConnectCallback("onConnectionSuspended try end:"), true);
    }
    ...
    public void onConnectionFailed(ConnectionResult result) {
        ...
        if (result == null) {
            ...
            onConnectEnd(YueJianAppHMSAgent.AgentResultCode.RESULT_IS_NULL);
            return;
        }

        int errCode = result.getErrorCode();
        YueJianAppHMSAgentLog.d("errCode=" + errCode + " allowResolve=" + allowResolveConnectError);

        if(HuaweiApiAvailability.getInstance().isUserResolvableError(errCode) && allowResolveConnectError) {
            Activity activity = YueJianAppActivityMgr.INST.getLastActivity();
            if (activity != null) {
                try {
                    timeoutHandler.sendEmptyMessageDelayed(APICLIENT_STARTACTIVITY_TIMEOUT_HANDLE_MSG, APICLIENT_STARTACTIVITY_TIMEOUT);
                    Intent intent = new Intent(activity, YueJianAppHMSAgentActivity.class);
                    intent.putExtra(YueJianAppHMSAgentActivity.CONN_ERR_CODE_TAG, errCode);
                    activity.startActivity(intent);
                    return;
                } catch (Exception e) {
                    YueJianAppHMSAgentLog.e("start YueJianAppHMSAgentActivity exception:" + e.getMessage());
                    timeoutHandler.removeMessages(APICLIENT_STARTACTIVITY_TIMEOUT_HANDLE_MSG);
                    onConnectEnd(YueJianAppHMSAgent.AgentResultCode.START_ACTIVITY_ERROR);
                    return;
                }
            } else {
                // 当前没有界面处理不了错误
                YueJianAppHMSAgentLog.d("no activity");
                onConnectEnd(YueJianAppHMSAgent.AgentResultCode.NO_ACTIVITY_FOR_USE);
                return;
            }
        } else {
            //其他错误码直接透传

...
public class XRoundImageView extends AppCompatImageView {
    ...
    public XRoundImageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.XRoundImageView, 0, 0);
        for (int i = 0; i < ta.getIndexCount(); i++) {
            int attr = ta.getIndex(i);
            if (attr == R.styleable.XRoundImageView_xriv_is_cover_src) {
                isCoverSrc = ta.getBoolean(attr, isCoverSrc);
            } else if (attr == R.styleable.XRoundImageView_xriv_is_circle) {
                isCircle = ta.getBoolean(attr, isCircle);
            } else if (attr == R.styleable.XRoundImageView_xriv_border_width) {
                borderWidth = ta.getDimensionPixelSize(attr, borderWidth);
            } else if (attr == R.styleable.XRoundImageView_xriv_border_color) {
                borderColor = ta.getColor(attr, borderColor);
            } else if (attr == R.styleable.XRoundImageView_xriv_inner_border_width) {
                innerBorderWidth = ta.getDimensionPixelSize(attr, innerBorderWidth);
            } else if (attr == R.styleable.XRoundImageView_xriv_inner_border_color) {
                innerBorderColor = ta.getColor(attr, innerBorderColor);
            } else if (attr == R.styleable.XRoundImageView_xriv_corner_radius) {
                cornerRadius = ta.getDimensionPixelSize(attr, cornerRadius);
            } else if (attr == R.styleable.XRoundImageView_xriv_corner_top_left_radius) {
                cornerTopLeftRadius = ta.getDimensionPixelSize(attr, cornerTopLeftRadius);
            } else if (attr == R.styleable.XRoundImageView_xriv_corner_top_right_radius) {
                cornerTopRightRadius = ta.getDimensionPixelSize(attr, cornerTopRightRadius);
            } else if (attr == R.styleable.XRoundImageView_xriv_corner_bottom_left_radius) {
                cornerBottomLeftRadius = ta.getDimensionPixelSize(attr, cornerBottomLeftRadius);
            } else if (attr == R.styleable.XRoundImageView_xriv_corner_bottom_right_radius) {
                cornerBottomRightRadius = ta.getDimensionPixelSize(attr, cornerBottomRightRadius);
            } else if (attr == R.styleable.XRoundImageView_xriv_mask_color) {
    ...
    public void setBorderWidth(int borderWidth) {
        this.borderWidth = (int) YueJianAppTDevice.dpToPixel(borderWidth);
    ...
    public void setInnerBorderWidth(int innerBorderWidth) {
        this.innerBorderWidth = (int) YueJianAppTDevice.dpToPixel(innerBorderWidth);
        clearInnerBorderWidth();
        invalidate();
    }
    ...
    public void setCornerRadius(int cornerRadius) {
        this.cornerRadius = (int) YueJianAppTDevice.dpToPixel(cornerRadius);
        calculateRadiiAndRectF(false);
    }

    public void setCornerTopLeftRadius(int cornerTopLeftRadius) {
        this.cornerTopLeftRadius = (int) YueJianAppTDevice.dpToPixel(cornerTopLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerTopRightRadius(int cornerTopRightRadius) {
        this.cornerTopRightRadius = (int) YueJianAppTDevice.dpToPixel(cornerTopRightRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomLeftRadius(int cornerBottomLeftRadius) {
        this.cornerBottomLeftRadius = (int) YueJianAppTDevice.dpToPixel(cornerBottomLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomRightRadius(int cornerBottomRightRadius) {
        this.cornerBottomRightRadius = (int) YueJianAppTDevice.dpToPixel(cornerBottomRightRadius);
        calculateRadiiAndRectF(true);
    }

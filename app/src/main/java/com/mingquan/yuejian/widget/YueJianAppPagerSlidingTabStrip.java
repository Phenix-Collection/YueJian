...
import com.mingquan.yuejian.interf.YueJianAppIPagerSlidingClickListener;
 ...
 */
public class YueJianAppPagerSlidingTabStrip extends HorizontalScrollView {
  public interface IconTabProvider { int getPageIconResId(int position); }
  ...
  private boolean smoothScrollWhenClickTab =
      true; //设置当点击TAB时,内容区域的ViewPager是否需要动画切换
  private List<Map<String, View>> tabViews = new ArrayList<>();
  private boolean mFadeEnabled = true;
  private float zoomMax = 0.3f;
  private State mState;
  private YueJianAppIPagerSlidingClickListener mPagerSlidingInterface;
  private boolean needRightArrow = false;
  private boolean mIsSetCustomWidth = false;

  private enum State { IDLE, GOING_LEFT, GOING_RIGHT }

  private int oldPage;

  public void setNeedRightArrow(boolean needRightArrow) {
    this.needRightArrow = needRightArrow;
  }

  public YueJianAppPagerSlidingTabStrip(Context context) {
    this(context, null);
  }

  public YueJianAppPagerSlidingTabStrip(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  @SuppressWarnings("ResourceType")
  public YueJianAppPagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
    ...
    dividerPaddingTopBottom =
        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerPaddingTopBottom, dm);
    ...
    a = context.obtainStyledAttributes(attrs, R.styleable.YueJianAppPagerSlidingTabStrip);

    indicatorColor =
        a.getColor(R.styleable.YueJianAppPagerSlidingTabStrip_pstsIndicatorColor, indicatorColor);
    underlineColor =
        a.getColor(R.styleable.YueJianAppPagerSlidingTabStrip_pstsUnderlineColor, underlineColor);
    dividerColor = a.getColor(R.styleable.YueJianAppPagerSlidingTabStrip_pstsDividerColor, dividerColor);
    indicatorHeight = a.getDimensionPixelSize(
        R.styleable.YueJianAppPagerSlidingTabStrip_pstsIndicatorHeight, indicatorHeight);
    underlineHeight = a.getDimensionPixelSize(
        R.styleable.YueJianAppPagerSlidingTabStrip_pstsUnderlineHeight, underlineHeight);
    dividerPaddingTopBottom = a.getDimensionPixelSize(
        R.styleable.YueJianAppPagerSlidingTabStrip_pstsDividerPaddingTopBottom, dividerPaddingTopBottom);
    tabPadding = a.getDimensionPixelSize(
        R.styleable.YueJianAppPagerSlidingTabStrip_pstsTabPaddingLeftRight, tabPadding);
    tabBackgroundResId =
        a.getResourceId(R.styleable.YueJianAppPagerSlidingTabStrip_pstsTabBackground, tabBackgroundResId);
    shouldExpand = a.getBoolean(R.styleable.YueJianAppPagerSlidingTabStrip_pstsShouldExpand, shouldExpand);
    scrollOffset =
        a.getDimensionPixelSize(R.styleable.YueJianAppPagerSlidingTabStrip_pstsScrollOffset, scrollOffset);
    textAllCaps = a.getBoolean(R.styleable.YueJianAppPagerSlidingTabStrip_pstsTextAllCaps, textAllCaps);
    selectedTabTextColor =
        a.getColor(R.styleable.YueJianAppPagerSlidingTabStrip_pstsTextSelectedColor, selectedTabTextColor);
    zoomMax = a.getFloat(R.styleable.YueJianAppPagerSlidingTabStrip_pstsScaleZoomMax, zoomMax);
    smoothScrollWhenClickTab = a.getBoolean(
        R.styleable.YueJianAppPagerSlidingTabStrip_pstsSmoothScrollWhenClickTab, smoothScrollWhenClickTab);
    a.recycle();
  ...
  private void notifyDataSetChanged() {
    ...
    if (mTitleViewList != null && mTitleViewList.size() > 0) {
  ...
  private boolean isMessageIconSet = false;

  public void setMessageIcon(boolean flag) {
    this.isMessageIconSet = flag;
  }
  ...
  private void addTextTab(final int position, String title) {
    ...
    /**当未被选中时不显示箭头*/
    /*    if (position == 1 && needRightArrow) {
//            Drawable rightDrawable = getResources().getDrawable(R.drawable.yue_jian_app_arrow_right);
            Drawable rightDrawable = getResources().getDrawable(R.drawable.yue_jian_app_arrow_bottom);
            rightDrawable.setBounds(0, 0, rightDrawable.getIntrinsicWidth(),
rightDrawable.getIntrinsicHeight());
            tab.setCompoundDrawablePadding((int) YueJianAppTDevice.dpToPixel(-10));
            tab.setCompoundDrawables(null, null, null, rightDrawable);
        }*/
    ...
    if (position == 2 && needRightArrow) {
      Drawable rightDrawable = getResources().getDrawable(R.drawable.yue_jian_app_arrow_bottom);
      rightDrawable.setBounds(
          0, 0, rightDrawable.getIntrinsicWidth(), rightDrawable.getIntrinsicHeight());
      tab2.setCompoundDrawablePadding((int) YueJianAppTDevice.dpToPixel(-10));
      tab2.setCompoundDrawables(null, null, null, rightDrawable);
    }
    mTitleViewList.add(position, tab2);
  ...
  private void addTab(final int position, View tab, View tab2) {
    ...
    /**设置小红点的Icon*/
    if (isMessageIconSet) {
      messageIcon = new ImageView(context);
      messageIcon.setImageResource(R.drawable.yue_jian_app_icon_new);
      ...
      framelayout.addView(messageIcon);
    }
    tabsContainer.addView(
        framelayout, position, shouldExpand ? expandedTabLayoutParams : defaultTabLayoutParams);
    ...
    if (isMessageIconSet && null != messageIcon) {
      map.put("message_icon", messageIcon);
    }
    tabViews.add(position, map);
  }

  public void setPagerSlidingListen(YueJianAppIPagerSlidingClickListener pagerSlidingListen) {
    this.mPagerSlidingInterface = pagerSlidingListen;
  }

  public List<Map<String, View>> getTabViews() {
    return tabViews;
  }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    float offset = 0f;
    if (mIsSetCustomWidth) { //设置自定义的Indicator宽度
      offset = setIndicatorWidth(currentTab, lineLeft, lineRight);
    }
    ...
    canvas.drawRect(
        lineLeft + offset, height - indicatorHeight, lineRight - offset, height, rectPaint);
    // draw divider
    dividerPaint.setColor(dividerColor);
    for (int i = 0; i < tabCount - 1; i++) {
      View tab = tabsContainer.getChildAt(i);
      canvas.drawLine(tab.getRight() - 20, dividerPaddingTopBottom, tab.getRight() - 20,
   ...
   * 是否设置下划线的宽度与title相等
   */
  public void setCustomIndicatorWidth(boolean flag) {
    this.mIsSetCustomWidth = flag;
  }
   ...
   * @return
   */
  private float setIndicatorWidth(View currentTab, float lineLeft, float lineRight) {
    float offset = 0f;
    float titleWidth = 0f;
    if (pager.getAdapter() != null && tabCount >= currentPosition) {
      /**设置indicator宽度*/
      FrameLayout mLayTitle = null;
      if (currentTab instanceof FrameLayout) {
        mLayTitle = (FrameLayout) currentTab;
      }
      View childView = mLayTitle.getChildAt(0);
      TextView mTitleText = null;
      if (childView != null && childView instanceof TextView) {
        mTitleText = (TextView) childView;
      }
      //            if (mLayTitle!=null&&mLayTitle instanceof TextView)
      TextPaint mTextPaint = null;
      if (mTitleText != null) {
        mTextPaint = mTitleText.getPaint();
      }
      titleWidth =
          mTextPaint.measureText(pager.getAdapter().getPageTitle(currentPosition).toString());
    }
    offset = ((lineRight - lineLeft) - titleWidth) / 2;
    return offset;
  }

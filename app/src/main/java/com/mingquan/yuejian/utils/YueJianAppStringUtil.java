 ...
 * @created 2012-3-21
 */
public class YueJianAppStringUtil {
   ...
   * 用户发言屏蔽开始
   *********/
  public static SpannableStringBuilder matchKey(String sendMsg) {
  ...
  public static String getGameId(String h5GameRes){
    ...
    YueJianAppTLog.error("name:"+name);
    String[] subStr=name.split("\\.");
    YueJianAppTLog.error("subStr.length:"+subStr.length);
    YueJianAppTLog.error("subStr.toString:"+subStr.toString());
    for (int i=0;i<subStr.length;i++) {
      YueJianAppTLog.error(subStr[i]);
    }
    String gameId=subStr[1];
    YueJianAppTLog.error("gameId:"+gameId);
    return gameId;
  }

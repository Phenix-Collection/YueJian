 ...
 */
public class YueJianAppTimeFormater {
   ...
   * @return
   */
  public static String getInterval2(String c) {
    String interval = null;
    long time = (System.currentTimeMillis() + 999) / 1000 - Long.parseLong(c);
    int i = (int) (time / (60 * 60 * 24));
    if (i < 1) {
      if (time < 0) {
  ...
  /**
   * 将时间戳转换为字符串 MM月d日 HH:mm
   *
   * @return
   */
  public static String timeToStrLong(long time) {
   ...
   * @return
   */
  public static String timeToStrLong2(long time) {
    Date dateDate = new Date(time);
   ...
   * @return
   */
  public static String timeToStrLines(int time) {
    Date dateDate = new Date(time * 1000);
    SimpleDateFormat formatter = new SimpleDateFormat("MM月d日\nHH:mm");
    String dateString = formatter.format(dateDate);
    return dateString;
  }
   ...
   * @return
   */
  public static String secondToStrLong(int time) {
    String timeStr = null;
    int hour = 0;
    int minute = 0;
    int second = 0;

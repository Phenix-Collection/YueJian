 ...
 */
public class YueJianAppCpu {
  static final String USER_CPU_REGEX =
      "((?i)(use?r\\s+[0-9]{1,2}(\\.[0-9]{1,2})?\\%))|([0-9]{1,2}\\.[0-9]{1,2}\\%\\s+(?i)(use?r))";
  static final String SYSTEM_CPU_REGEX =
      "((?i)(sys(tem)?\\s+[0-9]{1,2}(\\.[0-9]{1,2})?\\%))|([0-9]{1,2}\\.[0-9]{1,2}\\%\\s+(?i)(sys(tem)?))";
  ...
  public void parseTopResults(String name) {
    ...
    //        xRegexSearchPattern = Pattern.compile(YueJianAppCpu.USER_CPU_REGEX);
    ...
    //            try {
    //                this.m_fUserCpuUsage = Float.parseFloat( sUserCpuUsage.replaceAll( "[^\\d.]",
    //                "" ).trim() );
    //            }
    ...
    //        xRegexSearchPattern = Pattern.compile( YueJianAppCpu.SYSTEM_CPU_REGEX );
    //        xSearch = xRegexSearchPattern.matcher(this.m_sTopResults);
    //        //* check to see if we found a match
    //        if (xSearch.find())
    //        {
    ...
    //            //* we want to remove the non numeric characters in the string
    //            try {
    //                this.m_fSystemCpuUsage = Float.parseFloat( sSystemCpuUsage.replaceAll(
    //                "[^\\d.]", "" ).trim() );
    //            }
    //            catch (NumberFormatException exp)
    //            {

 ...
 * 身份证的工具类
 */
public class YueJianAppIdCardUtil {
     ...
     * @param idCardNum
     */
    public YueJianAppIdCardUtil(String idCardNum) {
        // super();
    ...
    public int isCorrect() {
        if (this.isEmpty()) {
            this.error = YueJianAppIdCardUtil.IS_EMPTY;
            return this.error;
        }

        if (!this.isLenCorrect()) {
            this.error = YueJianAppIdCardUtil.LEN_ERROR;
            return this.error;
        }

        if (!this.isCharCorrect()) {
            this.error = YueJianAppIdCardUtil.CHAR_ERROR;
            return this.error;
        }

        if (!this.isDateCorrect()) {
            this.error = YueJianAppIdCardUtil.DATE_ERROR;
            return this.error;
        }

        if (this.is18()) {
            if (!this.getCheck().equals(this.getCheckBit())) {
                this.error = YueJianAppIdCardUtil.CHECK_BIT_ERROR;
                return this.error;
            }
        }

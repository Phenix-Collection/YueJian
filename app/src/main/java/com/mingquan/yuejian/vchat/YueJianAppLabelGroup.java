...
public class YueJianAppLabelGroup extends ViewGroup implements View.OnClickListener {
    ...
    GradientDrawable normalBackgroundDrawable; // 默认label背景

    private boolean isSelectedAllLabels = true; // 选中所有的label
    ...
    public YueJianAppLabelGroup(Context context) {
        super(context);
        mContext = context;
    }

    public YueJianAppLabelGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        getAttrs(context, attrs);
    }

    public YueJianAppLabelGroup(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.YueJianAppLabelGroup);
            int type = mTypedArray.getInt(R.styleable.YueJianAppLabelGroup_selectType, 1);
            mSelectType = SelectType.get(type);

            mMaxSelect = mTypedArray.getInteger(R.styleable.YueJianAppLabelGroup_maxSelect, 0);
            mTextColor = mTypedArray.getColorStateList(R.styleable.YueJianAppLabelGroup_labelTextColor);
            mTextSize = mTypedArray.getDimension(R.styleable.YueJianAppLabelGroup_labelTextSize,
                    sp2px(context, 14));
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                    R.styleable.YueJianAppLabelGroup_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                    R.styleable.YueJianAppLabelGroup_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                    R.styleable.YueJianAppLabelGroup_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                    R.styleable.YueJianAppLabelGroup_labelTextPaddingBottom, 0);
            mLineMargin = mTypedArray.getDimensionPixelOffset(R.styleable.YueJianAppLabelGroup_lineMargin, 0);
            mWordMargin = mTypedArray.getDimensionPixelOffset(R.styleable.YueJianAppLabelGroup_wordMargin, 0);
            int labelBgResId = mTypedArray.getResourceId(R.styleable.YueJianAppLabelGroup_labelBackground, 0);
            if (labelBgResId != 0) {
                mLabelBg = getResources().getDrawable(labelBgResId);
                mLabelBg = getResources().getDrawable(labelBgResId);
            } else {
                int labelBgColor = mTypedArray.getColor(R.styleable.YueJianAppLabelGroup_labelBackground, Color.TRANSPARENT);
        ...
        normalBackgroundDrawable = new GradientDrawable();
        normalBackgroundDrawable.setColor(getResources().getColor(R.color.vchat_d6d6d6));
        normalBackgroundDrawable.setCornerRadius(sp2px(mContext, 14));
    }
...
//            }
    ...
    /*public void setLabels(List<String> labels) {
        setLabels(labels, new LabelTextProvider<String>() {
            @Override
            public CharSequence getLabelText(String data) {
                return data.trim();
            }

            @Override
            public String getLabelBackgroundColor(String data) {
                return null;
            }
        });
    }*/
     ...
     * @param <T>
     */
    public <T> void setLabels(List<YueJianAppACUserTagMetaDataModel> labels) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();
     ...
     * 不包含count
     */
    public void setLabels(ArrayList<YueJianAppUserTagAndCountModel> userTagAndCountModels) {
//清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();

        if (userTagAndCountModels != null) {
            mLabels.addAll(userTagAndCountModels);
            int size = userTagAndCountModels.size();
            for (int i = 0; i < size; i++) {
                addLabel(userTagAndCountModels.get(i), i);
            }
        }
    ...
    private <T> void addLabel(YueJianAppACUserTagMetaDataModel data, int position) {
        final TextView label = new TextView(mContext);
        boolean isNull = data == null;
        StateListDrawable sd = new StateListDrawable(); // 背景选择器
        GradientDrawable selectedDrawable = new GradientDrawable();// 选中状态的背景
        if (!isNull) {
            selectedDrawable.setColor(Color.parseColor(data.getColor()));
        }
        selectedDrawable.setCornerRadius(sp2px(mContext, 14));

        sd.addState(new int[]{android.R.attr.state_selected}, selectedDrawable);
        // 因为如果所有的标签都共用一个Drawable对象，会引起背景错乱。
        sd.addState(new int[]{}, normalBackgroundDrawable.getConstantState().newDrawable());

        if (mSelectType == SelectType.NONE) { // 只显示对应的背景色
            label.setBackgroundDrawable(selectedDrawable);
        } else {
            label.setBackgroundDrawable(sd); // 显示可变背景色
    ...
    private <T> void addLabel(YueJianAppUserTagAndCountModel data, int position) {
        final TextView label = new TextView(mContext);
        boolean isNull = data == null;
        StateListDrawable sd = new StateListDrawable(); // 背景选择器
        GradientDrawable selectedDrawable = new GradientDrawable();// 选中状态的背景
        if (!isNull) {
            selectedDrawable.setColor(Color.parseColor(data.getMetaDataModel().getColor()));
        }
        selectedDrawable.setCornerRadius(sp2px(mContext, 14));
        ...
        if (!isNull) {
            label.setTag(KEY_DATA, data);
            label.setText(String.format("%s  %s", data.getMetaDataModel().getName(), data.getCount()));
        }
        addView(label);
    }
    ...
    public interface LabelTextProvider<T> {
         ...
         * @return
         */
        CharSequence getLabelText(T data);

        String getLabelBackgroundColor(T data);
    }
}

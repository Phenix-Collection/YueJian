...
public class Texture2dProgram {
    ...
    public enum ProgramType {
        TEXTURE_2D, TEXTURE_EXT, TEXTURE_EXT_BW, TEXTURE_EXT_FILT, TEXTURE_MOSAIC
    }
    ...
    private static final String FRAGMENT_SHADER_MOSAIC =
            "precision mediump float;\n" +
                    "uniform sampler2D sTexture;\n" +
                    "varying vec2 vTextureCoord;\n" +
                    "\n" +
                    ...
                    "   float dx = pixel / 960.0;\n" +
                    "   float dy = pixel / 720.0;\n" +
                    "   vec2 coord = vec2(dx * floor(uv.x / dx) + 0.5 * dx, dy * floor(uv.y / dy) + 0.5 * dy);\n" +
                    "\n" +
                    "   vec2 from = vec2(960.0 * coord.x, 720.0 * coord.y);\n" +
                    "   vec2 to = vec2(960.0 * uv.x, 720.0 * uv.y);\n" +
                    "   float d = distance(from, to);\n" +
                    "\n" +
                    "   vec4 finalColor;\n" +
                    "   if(d < 0.5 * pixel) {\n" +
                    "      finalColor = texture2D(sTexture, coord);\n" +
                    "   } else {\n" +
    ...
    public Texture2dProgram(ProgramType programType) {
        ...
        switch (programType) {
            ...
            case TEXTURE_MOSAIC:
                mTextureTarget = GLES20.GL_TEXTURE_2D;
                mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, FRAGMENT_SHADER_MOSAIC);
                break;
            case TEXTURE_EXT:
